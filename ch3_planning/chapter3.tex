...
The project objectives and scope defined in Chapter~\ref{chap:introduction} remain aligned with findings synthesized in Chapter~\ref{chap:literature}. The execution environment has evolved: operations staff now have \texttt{docker} group access on Linux hosts, which relaxes several practical constraints that motivated tighter wording in earlier drafts (for example deployment of containerized collectors alongside production stacks). Core scope elements (heterogeneous on-premise \ac{CICD}, integrated metrics--logs--traces, empirical evaluation) are unchanged; requirements are updated to reflect the current access posture rather than the historically tighter one. The work focuses on execution strategy to deliver planned artifacts and produce defensible evidence.
...
\textbf{Non-functional requirements} include: operation in heterogeneous environments (Linux and Windows); reproducible deployment procedures; acceptable instrumentation overhead; operational maintainability for infrastructure teams; and feasibility under the access posture assumed for execution (operations accounts with \texttt{docker} group membership on Linux hosts, without requiring omnipotent root access across the estate unless explicitly approved for a given component).
